...
def create_dataloader(path,
                      ...
                      prefix='',
                      shuffle=False,
                      kgtb=False, # keep Ground-Truth-Box
                      seed=0):
    if rect and shuffle:
...
class LoadImagesAndLabels(Dataset):
    ...
    def load_mosaic(self, index):
        ...
        labels4, segments4 = [], []
        s = self.img_size
        if self.kgtb: # mosaicの中心を動かさないために固定する
            yc, xc = s, s
        else:            
        ...
        #print('labels4:', labels4) #debugprint cls,xyxy（比率ではなくpixel位置）で返されている
        # Augment # 以下のcopy_pasteとrandom_perspectiveを実行するとグランドトゥルースボックスが分断されてしまうためコメントアウトしている
        if self.kgtb:
            img4, labels4 = self.random_crop_keepGTB(img4, labels4)
        else:
        ...
        #print('img4.shape:', img4.shape) #debugprint
        #print('labels4:', labels4) #debugprint cls,xyxy（比率ではなくpixel位置）で返されている
        return img4, labels4
