...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, kgtb=False, imgsz=640, p=1.0, n=4):
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            mosaic_labels.append(labels_patch)
        #print('type(mosaic_labels):', type(mosaic_labels), [len(v) for v in mosaic_labels])
        final_labels = self._cat_labels(mosaic_labels)
        final_labels["img"] = img4
        #print('img4.shape:', img4.shape) # ここではimg4.shape: (1280, 1280, 3)
        #print('final_labels:', final_labels["instances"]._bboxes.bboxes)
        #final_labels: [[165? 434  229  473][993  457  1048  492][1164.4  1012.2  1209.9  1034.9]]?????
        return final_labels
...
class keepGTB:
    ...
    def __call__(self, labels):
        ...
        while True:
            ...
            for i, label in enumerate(labelset):
                if float(label[2]) <= left or float(label[0]) >= right or float(label[3]) <= top or float(label[1]) >= bottom:
                    ...
                    label[2] = label[2] - left
                    label[1] = label[1] - top
                    label[3] = label[3] - top
                    ll.append(label)
                    classes.append(cls[i])
            if len(ll) > 0 and len(classes) > 0:
                break
            cnt += 1
            '''
            YOLOv8ではグランドトゥルースボックスが一つもない完全バックグラウンドデータを学習に使うことはできない様子
            このため無限ループに陥る可能性が残るので10000回試行してもGTBを含む640×640が取得できない場合
        ...
        #print('LTRB & labels:', left, top, right, bottom, labelset) #debugprint
        labels["instances"]._bboxes.bboxes = np.array(ll)
        labels["cls"] = np.array(classes)        
        labels["img"] = img
        return labels
